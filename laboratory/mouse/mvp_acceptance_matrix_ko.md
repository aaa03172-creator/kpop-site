...
이 문서는 현재 MVP가 handwritten cage-card 기반 colony tracking의 핵심 흐름을 어느 정도 충족하는지 확인하기 위한 acceptance matrix이다. Canonical 제품 요구사항은 `final_mouse_colony_prd.md`와 `AGENTS.md`를 따른다.
...
| --- | --- | --- | --- | --- |
| A01 | Source Photo | 사진 업로드는 raw source로 저장되고 원본 bytes/image endpoint로 조회된다. | Done | `scripts/verify-local-app.py` photo upload/image assertions |
| A02 | Photo Review | 업로드된 사진은 transcription review candidate를 생성하고, 승인 기반 AI extraction은 reviewable parsed evidence로 저장한다. | Done | `Photo review candidate should be visible in Review Queue`, `AI extraction should save parsed note evidence` |
...
| A11 | Apply Preview | canonical candidate apply preview는 write 대상 mouse/event/duplicate risk를 보여주고 state를 바꾸지 않는다. | Done | `/apply-preview` no-write assertions |
...
| A14 | Cage Movement | cage move는 current assignment와 movement event/action을 함께 기록한다. | Done | `mouse_cage_moved`, active/ended assignment assertions |
...
| Gap ID | 내용 | 권장 다음 작업 |
| --- | --- | --- |
| G01 | 실제 브라우저에서 fixture가 아닌 사용자가 사진 업로드, AI extraction, review resolve, apply/export까지 수행하는 full E2E 자동화가 아직 약하다. | Playwright 또는 in-app browser 기반 photo extraction E2E 추가 |
| G02 | Review detail은 evidence를 보여주지만 crop/ROI 수준 이미지 근거는 아직 제한적이다. | note line crop 또는 photo annotation 후보 검토 |
| G03 | Configurable masters 일부는 구현되었지만 status/event/genotype category 전체가 master화된 것은 아니다. | controlled vocabulary registry 추가 |
| G04 | Experiment traceability는 큰 설계에는 있으나 MVP 자동 흐름에는 아직 약하다. | experiment-mouse link MVP 추가 |
