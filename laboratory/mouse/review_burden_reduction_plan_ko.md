...
Data boundary: review item design and export/view UX planning.

Canonical status: non-canonical. This document plans ways to reduce review burden after cage-card OCR/ROI extraction. It does not replace `final_mouse_colony_prd.md`, `AGENTS.md`, or adopted project documents, and it does not define canonical schema or API behavior by itself.
...
현재 시스템은 불확실성을 보존하는 방향으로 발전했지만, 사용자에게 보이는 Review Queue는 아직 "모든 불확실성"을 "즉시 사람이 확인해야 하는 일"처럼 보여준다. 이 구조는 traceability에는 유리하지만 실제 사용성에는 부담이 크다.

목표는 review item 개수를 무작정 줄이는 것이 아니다. 원본 사진과 raw evidence는 계속 보존하되, 사용자가 지금 봐야 하는 위험 항목과 나중에 참고해도 되는 trace 항목을 분리하는 것이다.

## 현재 상태 요약

현재 open review item은 40개다.

| 유형 | 개수 | 현재 문제 |
| --- | ---: | --- |
| AI-extracted photo transcription needs review | 17 | 사진마다 1개씩 올라와 모든 사진을 확인해야 하는 것처럼 보인다. |
| Unlabeled numeric note needs review | 12 | 숫자 note가 줄마다 review로 쪼개져 반복 작업처럼 보인다. |
| Outside assigned strain scope | 5 | fixture/sample 데이터가 실제 업무 review처럼 섞여 보인다. |
| Ear label needs review | 4 | 필요한 확인이지만 사진 단위 review와 분리되어 맥락이 약하다. |
| Duplicate active mouse | 1 | 실제 high priority로 유지해야 한다. |
| Low-confidence strain alias | 1 | strain curator가 볼 수 있으나 기본 작업 화면에 앞설 필요는 낮다. |

최근 AI 추출 17건 기준:

| 분류 | 개수 | 의미 |
| --- | ---: | --- |
| assigned strain 자동 매칭 | 13 | `ApoMtg/tg` 등 alias 기반 정리가 가능하다. |
| strain attention 필요 | 4 | 빈 strain, `ApoM 7`, `Atg4M Tg/Tg` 등이다. |
| high attention | 6 | 낮은 confidence, 빈 strain/sex, 핵심값 누락이 있다. |
| medium attention | 6 | notes/count/DOB 일부 확인이 필요하다. |
| light attention | 5 | raw value와 주요 추출값은 대체로 충분하다. |

## 사용자 피로를 만드는 원인

1. Review Queue가 너무 평평하다.

   High risk와 light uncertainty가 모두 같은 "open review"처럼 보인다. 사용자는 무엇부터 봐야 하는지 알기 어렵고 모든 항목을 확인해야 한다고 느낀다.
...
4. DOB normalization 불확실성이 과도하게 전면화된다.
   ...
   raw DOB가 보존되어 있고 canonical write 전까지 정규화가 필요하지 않은 경우에도 review blocker처럼 보일 수 있다.

5. 자동 매칭된 값도 계속 의심하게 만든다.

   assigned strain alias로 exact/high-confidence match된 경우는 "확인 완료에 가까운 draft"로 보여야 한다. 사용자가 다시 검토해야 하는 일처럼 보이면 피로가 커진다.
...
2. Canonical write 전에 위험 항목만 막는다.
...
4. Light uncertainty는 숨기지 않고 뒤로 보낸다.

   로그, badge, trace detail에는 남기되 기본 queue에서 사용자를 압박하지 않는다.

5. 사람의 일은 "수정"보다 "확인/예외 처리"가 기본이어야 한다.

   대부분의 사진은 괜찮으면 넘어갈 수 있어야 한다. 반대로 고위험 항목은 명확하게 드러나야 한다.

## 제안하는 Review 레벨

### Level 1. Must Review

사용자가 반드시 봐야 하는 항목이다. export 또는 canonical candidate 생성을 막는 방향을 검토할 수 있다.
...
한 번 훑으면 되는 항목이다. 기본 queue에는 약하게 보이거나 quick check 그룹으로 보여야 한다.

조건 예시:

- assigned strain이 자동 매칭됨
- sex/count가 있음
- raw DOB는 보존되었지만 normalized DOB가 비어 있음
- LMO 체크가 불확실함
- note line 일부가 애매하지만 canonical write에 직접 이어지지 않음

현재 다수의 `ApoMtg/tg` 자동 매칭 사진이 여기에 해당한다.

### Level 3. Trace Only

사용자에게 즉시 확인을 요구하지 않는다. trace detail이나 audit log에는 남긴다.

조건 예시:

- raw DOB가 있고 normalized DOB만 비어 있음
- assigned strain exact alias match
- LMO raw가 비어 있지만 현재 workflow에서 canonical write에 영향이 없음
- card type은 unknown이지만 strain/sex/count/note continuity에는 영향이 낮음

### Level 4. Hidden By Default

기본 사용자 workflow에서는 숨긴다.
...
review item을 나열하기보다 photo card에 다음 정보를 요약한다.
...
현재 숫자 note는 개별 review로 생성된다. 예를 들어 `1`, `2`, `3`, `4`, `5`가 각각 review가 된다. 이를 사진 단위 하나로 묶는다.
...
- `matchedStrain`이 active assigned strain의 exact alias match
- sex symbol과 count가 존재
- raw DOB가 존재
- confidence >= 65
- note line이 canonical mouse creation에 직접 이어지지 않음
...
아래 항목은 raw evidence가 보존되어 있으면 기본 queue에서 강하게 압박하지 않아야 한다.
...
3. Review Queue 기본 필터를 Focus Review로 변경
   ...
   기본 표시: must_review만
   선택: quick_check 포함, all review 포함

4. numeric note review 그룹화

   한 review 생성은 사진/parse 단위로 묶는다. 기존 개별 review는 supersede하거나 hidden_default로 내린다.
...
2. 사진 한 장에 대한 같은 맥락의 review를 여러 번 열지 않는다.
3. raw photo, raw OCR, before/after correction은 계속 trace 가능하다.
4. high-risk canonical write는 사전 review 없이 진행되지 않는다.
5. 사용자가 "확인해야 할 것"과 "참고로 남긴 것"을 한눈에 구분할 수 있다.

## 결론

현재 extraction/ROI/matching 자체는 점점 좋아지고 있지만 review UX는 아직 너무 보수적이다. 다음 제품 개선은 정확도 자체보다 "확인 부담을 줄이는 설계"가 우선이다.
