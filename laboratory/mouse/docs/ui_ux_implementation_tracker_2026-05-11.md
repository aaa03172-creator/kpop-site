...
| Brand and color direction | Documented. | #111. | Consolidate semantic CSS tokens across chips, status pills, selected cards, disabled controls, ready/blocked states, and focus rings. |
| Empty/loading/error state | Partially documented. | #111. | Normalize copy and layout for Photo Review, Review Queue, Evidence Ledger, Export Center, and read-model panels. |
...
1. Empty/loading/error state standardization.
   - Scope: static UI copy and structure only, no new API behavior.
   - Boundary: export or view / review item display states.
   - Verification: `npm test`, `npm run test:local`, browser checks for honest
     loading/empty/error copy without fabricated mouse IDs, strains, dates, or
     review counts.

2. Semantic color token consolidation.
   - Scope: CSS token cleanup for review, warning, success, disabled, selected,
     ready, blocked, and focus states.
   - Boundary: design guidance / UI implementation detail.
   - Verification: `npm test`, `npm run test:local`, browser screenshots or DOM
     checks for status classes and accessible text labels.

3. Export preview row-state chips.
